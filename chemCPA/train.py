...
def set_up_covariates(dataset, idx_treated, n_idx_ctrl):
    ...
    if dataset.drugs_idx is not None:
        ...
        drugs = (
            [dataset.drugs_idx[idx_treated]] * n_idx_ctrl,
    ...
    if dataset.knockouts_idx is not None:
        knockouts = (
            [dataset.knockouts_idx[idx_treated]] * n_idx_ctrl,
...
def evaluate_logfold_r2(autoencoder, ds_treated: SubDataset, ds_ctrl: SubDataset, return_mean: bool = True):
    logfold_score = []
    signs_score = []
    ...
    for cell_pert_comb, category_count in zip(*np.unique(ds_treated.pert_categories, return_counts=True)):
        ...
        bool_de = ds_treated.var_names.isin(np.array(ds_treated.de_genes[cell_pert_comb]))
...
def evaluate_disentanglement(autoencoder, data):
    ...
    with torch.no_grad():
        _, latent_basal = autoencoder.predict(
            ...
            drugs_embeddings=(
                [_move_inputs(data.drugs_embeddings(i))[0] for i in data.drugs_idx]
                if data.drugs_idx is not None
                else None
            ),
            knockouts_idx=_move_inputs(data.knockouts_idx)[0],
            knockouts_embeddings=(
                [_move_inputs(data.knockouts_embeddings(i))[0] for i in data.knockouts_idx]
                if data.knockouts_idx is not None
                else None
            ),
    ...
    def compute_score(labels):
        ...
        labels_tensor = torch.tensor([label_to_idx[label] for label in labels], dtype=torch.long, device=_device)
...
def evaluate_r2(autoencoder, dataset: SubDataset, genes_control: torch.Tensor, return_mean: bool = True):
    ...
    for cell_pert_comb, category_count in zip(*np.unique(dataset.pert_categories, return_counts=True)):
        ...
        bool_de = dataset.var_names.isin(np.array(dataset.de_genes[cell_pert_comb]))
        idx_de = bool2idx(bool_de)
        ...
        mean_pred, var_pred = compute_prediction(autoencoder, genes_control, drugs, knockouts, covs)
    ...
    if return_mean:
        ...
        return [mean_score, mean_score_de, var_score, var_score_de]
...
def evaluate_r2_sc(autoencoder, dataset: SubDataset, return_mean: bool = True):
    ...
    for cell_pert_comb, category_count in zip(*np.unique(dataset.pert_categories, return_counts=True)):
        ...
        mean_score.append(r2_m) if not r2_m == float("-inf") else inf_combinations.add(cell_pert_comb)
        var_score.append(r2_v) if not r2_v == float("-inf") else inf_combinations.add(cell_pert_comb)
        mean_score_de.append(r2_m_de) if not r2_m_de == float("-inf") else inf_combinations.add(cell_pert_comb)
        var_score_de.append(r2_v_de) if not r2_v_de == float("-inf") else inf_combinations.add(cell_pert_comb)
...
def evaluate(
    ...
    if run_disentangle:
        ...
        if datasets["test"].num_knockouts > 0:
            knockouts_names, knockouts_counts = np.unique(datasets["test"].knockouts_names, return_counts=True)
    ...
    if run_r2_sc:
        ...
        test_sc = evaluate_r2_sc(autoencoder, datasets["test_treated"])
        if test_sc != []:
    ...
    if run_logfold:
        ...
        evaluation_stats["training_logfold_score"], evaluation_stats["training_logfold_signs_score"] = (
            evaluate_logfold_r2(
                autoencoder,
                datasets["training_treated"],
