...
class Dataset:
    ...
    def __init__(
        ...
        if drug_key is not None:
            ...
            self.drugs_idx = torch.from_numpy(np.array(drugs_idx))
            ...
            self.dosages = torch.from_numpy(np.array(dosages))
            ...
            if isinstance(drugs_embeddings, torch.nn.Embedding):
                ...
                drugs_embeddings = torch.tensor(
                    drugs_embeddings_df.loc[self.canon_smiles_unique_sorted].values, dtype=torch.float32
                )
        ...
        if knockout_key is not None:
            ...
            if isinstance(knockouts_embeddings, dict):
                self.knockouts_embeddings = {
                    self._knockouts_name_to_idx[name]: knockouts_embeddings[name]
                    for name in self.knockouts_names_unique_sorted
                }
                ...
                self.knockouts_embeddings = {
                    self._knockouts_name_to_idx[name]: self.knockouts_embeddings[name]
                    for name in self.knockouts_names_unique_sorted
                }
            else:
                # maybe provided with None, create random embeddings
                self.knockouts_embeddings = {
                    self._knockouts_name_to_idx[name]: torch.randn(256) for name in self.knockouts_names_unique_sorted
                }
            self.knockouts_embeddings = torch.nn.Embedding.from_pretrained(
                torch.stack([self.knockouts_embeddings[i] for i in range(self.num_knockouts)]), freeze=True
            )
        ...
        if isinstance(covariate_keys, list) and len(covariate_keys) > 0:
            ...
            for cov in covariate_keys:
                ...
                self.covariates_idx.append(covariate_idx)
            self.covariates_idx = torch.tensor(self.covariates_idx).long()
    ...
    def __getitem__(self, i):
        return (
            ...
            self.drugs_idx[i],
            self.dosages[i],
            self.drugs_embeddings(self.drugs_idx[i]),
            None,
            None,
            *[cov[i] for cov in self.covariates_idx],
        )
...
class SubDataset:
    ...
    def __init__(self, dataset: Dataset, indices):
        ...
        self.genes = indx(dataset.genes, indices)
        self.drugs_idx = indx(dataset.drugs_idx, indices)
        self.dosages = dataset.dosages[indices]
        self.knockouts_idx = indx(dataset.knockouts_idx, indices)
        self.covariates_idx = (
            [cov[indices] for cov in dataset.covariates_idx] if dataset.covariates_idx is not None else None
        )
