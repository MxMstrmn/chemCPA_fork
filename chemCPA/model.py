...
from chemCPA.train import evaluate, evaluate_logfold_r2, evaluate_r2, evaluate_r2_sc
...
class ComPert(L.LightningModule):
    ...
    def __init__(
        ...
        self.basal_state_added_noise_std = basal_state_added_noise_std
        self.basal_state_regularization = basal_state_regularization

        def perturbation_effect_size_mlp_MLP():
            return MLP(
                [1] + [self.hparams.hparams["dosers_width"]] * self.hparams.hparams["dosers_depth"] + [1],
                batch_norm=False,
            )

        def perturbation_effect_size_amortized_MLP(input_embedding_dimension):
            return MLP(
                [input_embedding_dimension + 1]
        ...
        if self.num_drugs > 0: 
            ...
            self.drug_embedding_encoder = MLP(
                ...
                + [self.hparams.hparams["embedding_encoder_width"]] * self.hparams.hparams["embedding_encoder_depth"]
                + [self.hparams.hparams["dim"]],
                last_layer_act="linear",
            )
        ...
        if self.num_covariates == [0]:
            ...
            self.covariates_embeddings = (
                []
            )  # TODO: Continue with checking that dict assignment is possible via covariate names and if dict are possible to use in optimisation
    ...
    def predict(
        ...
        if self.num_drugs > 0:
            drug_embedding = self.compute_drug_and_knockout_embeddings_(
                ...
                dosers=self.dosers,
                encoder=self.drug_embedding_encoder,
            )
        ...
        if self.num_knockouts > 0:
            ...
            knockout_embedding = self.compute_drug_and_knockout_embeddings_(
                knockouts_idx,
                effects,
                knockouts_embeddings,
                self.knockout_effect_type,
                self.knockout_effects,
                self.knockout_embedding_encoder,
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.run_adv and train_drugs and train_knockouts:
            optimizer_autoencoder, optimizer_adversaries, optimizer_dosers, optimizer_knockout_effects = (
                self.optimizers()
            )
            scheduler_autoencoder, scheduler_adversary, scheduler_dosers, scheduler_knockout_effects = (
                self.lr_schedulers()
            )
        elif self.run_adv and train_drugs:
        ...
        elif self.run_adv and train_knockouts:
        ...
        if self.run_adv:
            ...
            if self.num_knockouts > 0:
                ...
                adversary_knockouts_loss = self.loss_adversary_knockout(adversary_knockouts_predictions, knockouts_idx)
        ...
        if not self.hparams.training_params["full_eval_during_train"]:
            ...
            if self.hparams.training_params["run_eval_r2"]:
                self.log(
                    ...
                    np.mean(
                        [
                            evaluation_stats["test_mean_score"],
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        if self.run_adv:
            checkpoint["state_dict_covs"]["adversary_covariates"] = [
                adversary_covariates.state_dict() for adversary_covariates in self.adversary_covariates
            ]
        checkpoint["state_dict_covs"]["covariates_embeddings"] = [
            covariate_embedding.state_dict() for covariate_embedding in self.covariates_embeddings
        ]
