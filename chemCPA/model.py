...
class ComPert(L.LightningModule):
    ...
    def __init__(
        ...
        self.train_adversarial = train_adversarial
        ...
        if self.train_adversarial and self.num_covariates != [0]:
    ...
    def predict(
        ...
        if not self.train_adversarial:
            latent_treated = latent_basal + torch.randn(latent_basal.shape).to(self.device)*self.basal_state_added_noise_std
        else:
